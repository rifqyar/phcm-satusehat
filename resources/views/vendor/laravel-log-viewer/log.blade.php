      ...
      @else
        <table id="table-log" class="table table-striped" data-ordering-index="{{ $standardFormat ? 2 : 0 }}">
            ...
            @foreach($logs as $key => $log)
                @php
                    $logText = $log['text'] ?? '';
                    $jsonStart = strpos($logText, '{');
                    $title = $logText;
                    $jsonData = null;

                    if ($jsonStart !== false) {
                        $title = trim(substr($logText, 0, $jsonStart)); // teks sebelum JSON
                        $jsonString = substr($logText, $jsonStart);
                                ...
                                @foreach($jsonData as $jsonKey => $jsonValue)
                                    @if(is_array($jsonValue))
                                        <details style="margin-bottom: 6px;">
                                            ...
                                            <pre style="background:#f8f9fa; padding:10px; border-radius:8px; margin-top:5px; white-space:pre-wrap;">
{{ json_encode($jsonValue, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) }}
                                            </pre>
                                        </details>
                                    @else
                                        <div style="margin-bottom: 4px;">
                                            <strong>{{ ucfirst($jsonKey) }}:</strong>
                                            <span>{{ is_string($jsonValue) ? $jsonValue : json_encode($jsonValue) }}</span>
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                        @endif
                        {{-- ===== Akhir custom tampilan ===== --}}
