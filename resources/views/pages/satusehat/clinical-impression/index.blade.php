...
@push('after-script')
    ...
    <script>
        ...
        function lihatDetail(param) {
            ...
            ajaxGetJson(
                `{{ route('satusehat.clinical-impression.lihat-detail', '') }}/${btoa(param)}`,
                "show_modal",
                ""
            );
        }
        ...
        function show_modal(response) {
            ...
            $('#prognosis').val(response.dataErm.PROGNOSIS).prop('disabled', true);

            // Show status based on integration status
            $('#integrasi_prognosis, #success_prognosis, #failed_prognosis').hide();

            if (!response.dataPasien.statusIntegrated) {
                $('#integrasi_prognosis').show();
            } else if (response.dataPasien.statusIntegrated) {
                $('#success_prognosis').show();
            }
        ...
        function sendSatuSehat(param) {
            ...
            Swal.fire({
                ...
                text: `Kirim data Impresi Klinis Pasien ke SatuSehat?`,
                icon: "question",
                ...
                if (conf.value || conf.isConfirmed) {
                    await ajaxPostFile(
                        `{{ route('satusehat.clinical-impression.send') }}`,
                        formData,
                        "input_success",
                    );
                }
        ...
        function resendSatuSehat(param) {
            ...
            Swal.fire({
                ...
                text: `Kirim ulang data Impresi Klinis Pasien?`,
                icon: "question",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                ...
                if (conf.value || conf.isConfirmed) {
                    await ajaxPostFile(
                        `{{ route('satusehat.clinical-impression.resend', '') }}`,
                        formData,
                        "input_success",
                    );
                }
        ...
        function bulkSend() {
            ...
            Swal.fire({
                ...
                text: `Kirim ${selectedIds.length} data Impresi Klinis ke SatuSehat?`,
                icon: "question",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                ...
                if (result.value) {
                    await ajaxPostJson(`{{ route('satusehat.clinical-impression.bulk-send') }}`, {
