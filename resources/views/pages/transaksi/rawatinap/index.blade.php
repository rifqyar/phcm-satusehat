...
@section('content')
    <div class="row page-titles">
        ...
        <div class="col-md-5 col-8 align-self-center">
            <h3 class="text-themecolor">Transaksi Rawat Inap Satusehat</h3>
            <ol class="breadcrumb">
                <li class="breadcrumb-item">Transaksi Satusehat</li>
                <li class="breadcrumb-item active"><a href="{{ route('transaction.rawat-inap.index') }}">Rawat Inap</a>
                </li>
            </ol>
        </div>
    ...
    <script>
        ...
        function getAllData() {
            table = $('.data-table').DataTable({
                ...
                scrollX: false,
                ajax: {
                    url: `{{ route('transaction.rawat-inap.datatable') }}`,
                    method: "POST",
                    data: function(data) {
        ...
        function lihatDetail(param) {
            ...
            ajaxGetJson(
                `{{ route('transaction.rawat-inap.lihat-detail', '') }}/${btoa(param)}`,
                "show_modal",
                ""
            );
        }
        ...
        function sendSatuSehat(param) {
            ...
            Swal.fire({
                ...
                if (conf.value || conf.isConfirmed) {
                    await ajaxPostFile(
                        `{{ route('transaction.rawat-inap.send-satusehat') }}`,
                        formData,
                        "input_success",
                    );
                }
        ...
        function resendSatuSehat(param) {
            ...
            Swal.fire({
                ...
                if (conf.value || conf.isConfirmed) {
                    await ajaxPostFile(
                        `{{ route('transaction.rawat-inap.resend-satusehat', '') }}`,
                        formData,
                        "input_success",
                    );
                }
        ...
        function bulkSend() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    await ajaxPostJson(`{{ route('transaction.rawat-inap.bulk-send-satusehat') }}`, {
        ...
        $('#service').on('change', function() {
            ...
            ajaxGetJson(`{{ route('transaction.rawat-inap.get-log', '') }}/${paramSatuSehat}/${serviceName}`,
                "show_log", "")
        });
