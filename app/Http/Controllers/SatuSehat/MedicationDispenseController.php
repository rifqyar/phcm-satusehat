...
class MedicationDispenseController extends Controller
{
    ...
    public function createMedicationRequestPayloadfromDispense(Request $request)
    {
        try {
            ...
            if (empty($idTrans) || empty($kdbrg)) {
                return response()->json(['status' => 'error', 'message' => 'Parameter idTrans dan kdbrg wajib dikirim'], 400);
            }

            $result = $this->buildMedicationRequestAndDispatch($idTrans, $kdbrg);
            return $result['error'] ? $result['response'] : response()->json(['status' => 'success', 'message' => 'MedicationRequest dari Dispense dikirim ke antrian']);
        } catch (\Throwable $e) {
    ...
    private function buildMedicationRequestAndDispatch($idTrans, $kdbrg)
    {
        // Query data
            ...
            if (empty($item->medicationReference)) {
                return [
                    ...
                    'response' => response()->json(
                        [
                            'status' => 'error',
                            'message' => 'Kode FHIR tidak didapat dari SATUSEHAT',
                        ],
                        500,
                    ),
                ];
