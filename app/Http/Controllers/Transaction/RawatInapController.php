...
class RawatInapController extends Controller
{
    public function index()
    {
        ...
        return view('pages.transaksi.rawatinap.index', compact('result', 'satuSehatMenu', 'listService'));
    }
    ...
    public function datatable(Request $request)
    {
        ...
        foreach ($dataKunjungan as $row) {
            $jenis = $row->JENIS_PERAWATAN == 'RAWAT_INAP' ? 'RI' : 'RJ';
    ...
    public function lihatDetail($param)
    {
        ...
        $data = collect(DB::select("SELECT DISTINCT * FROM dbo.fn_getDataKunjungan(?, 'RAWAT_INAP') where ID_TRANSAKSI = ?", [
            $id_unit,
            $param
        ]))->first();
    ...
    public function getLog($param, $service)
    {
        $id_unit = Session::get('id_unit', '001');
        $karcis = $param;
        $encounter = DB::select("SELECT TOP 1 * FROM dbo.fn_getDataKunjungan(?, 'RAWAT_INAP') where ID_TRANSAKSI = ?", [
            $id_unit,
            $karcis
        ]);
    ...
    public function sendSatuSehat(Request $request)
    {
        try {
            ...
            $post = [
                ...
                'aktivitas' => 'RAWAT INAP',
                'jenis_layanan' => 'Rawat Inap, RAWAT_INAP, RAWAT INAP',
            ...
            $postBase = $post;
            $postBase['aktivitas'] = 'KIRIM TRANSAKI RAWAT INAP ALL IN';
            ...
            $icd9 = DB::selectOne("SELECT ICD9, DIPLAY_ICD9 FROM fn_getDataKunjungan(?, 'RAWAT_INAP') WHERE ID_TRANSAKSI = ?", [$id_unit, $arrParam['id_transaksi']]);
