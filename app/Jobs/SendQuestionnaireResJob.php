...
class SendQuestionnaireResJob implements ShouldQueue
{
  ...
  {
    $this->param = $param;
    $this->onQueue('QuestionnaireResponse');
  }
  ...
  public function handle()
  {
    ...
    try {
      ...
      $controller = app(QuestionnaireResponseController::class);
      ...
      Log::info('Send Questionnaire Response job completed successfully', [
        'param' => $this->param,
        'status_code' => $result->getStatusCode(),
      ]);
    } catch (Exception $e) {
      // Log error, job will be retried according to queue config
      Log::error('Send Questionnaire Response job failed: ' . $e->getMessage(), [
        'param' => $this->param,
